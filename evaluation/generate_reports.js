...
const taskAnalysis = JSON.parse(
  fs.readFileSync("evaluation/task_analysis.json", "utf8")
);
const surveyMapping = JSON.parse(
  fs.readFileSync("evaluation/survey_mapping.json", "utf8")
);
const examples = JSON.parse(
  fs.readFileSync("evaluation/examples_good_bad.json", "utf8")
);
...
Object.keys(taskAnalysis).forEach((branch) => {
  ...
  let v = 0;
  tasks.forEach((t) => {
    if (taskAnalysis[branch][t].verified) v++;
  });
...
Object.keys(groups).forEach((g) => {
  csvGroup += `${g},${groups[g].count},${(
    groups[g].verifiedSum / groups[g].count
  ).toFixed(2)}\n`;
});
