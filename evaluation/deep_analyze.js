...
const xlsxPath = "/home/cgoek/Studium/BA/buyin-todo-sandbox/thesis/surveys/Probanden-Einstufungsformular (Responses).xlsx";

function parseSurveyData() {
  try {
    const sharedStringsXml = runCommand(`unzip -p "${xlsxPath}" xl/sharedStrings.xml`);
    const sharedStrings = [];
    ...
    const sheetXml = runCommand(`unzip -p "${xlsxPath}" xl/worksheets/sheet1.xml`);
...
console.log(`Loaded survey data for ${Object.keys(surveyData).length} participants.`);
...
chatFiles.forEach(file => {
  ...
  if (matchedBranch) {
    chatAnalysis[matchedBranch] = {
      ...
      estimatedPrompts: promptCount > 0 ? promptCount : Math.ceil(wordCount / 100),
...
for (const branch of branches) {
  ...
  try {
    ...
    if (branch.startsWith('origin/')) {
        execSync(`git checkout ${branchName} 2>/dev/null || git checkout -b ${branchName} ${branch} 2>/dev/null`, { stdio: 'ignore' });
    } else {
    ...
    if (currentBranch === branchName) {
      ...
      console.log(`  Checkout mismatch: Wanted ${branchName}, got ${currentBranch}`);
    }
  } catch (e) {
  ...
  if (checkoutSuccess) {
    tasks = {
      T1_Persistence: checkFileContent('backend/src/repositories/TodoRepository.ts', 'save') || checkFileContent('backend/src/repositories/TodoRepository.ts', 'create'),
      T2_Auth: fs.existsSync('backend/src/middleware/auth.ts') || fs.existsSync('backend/src/controllers/AuthController.ts'),
      T3_Categories: fs.existsSync('backend/src/models/Category.ts') || fs.existsSync('frontend/src/features/categories'),
      T4_Attachments: fs.existsSync('backend/src/models/Attachment.ts') || fs.existsSync('backend/src/repositories/AttachmentRepository.ts'),
      T5_Calendar: fs.existsSync('frontend/src/features/calendar/CalendarView.tsx'),
      T6_Email: fs.existsSync('backend/src/services/EmailService.ts'),
      T7_Performance: checkFileContent('backend/src/controllers/TodoController.ts', 'performance') || fs.existsSync('backend/src/utils/performance.ts'),
      T8_ICS: fs.existsSync('backend/src/services/IcsService.ts') || fs.existsSync('todos-test.ics')
    };
    
    const tsFiles = runCommand('find . -name "*.ts" -not -path "*/node_modules/*"').split('\n').filter(Boolean);
    const anyCount = runCommand('grep -r ": any" backend/src frontend/src | wc -l');
    const consoleCount = runCommand('grep -r "console.log" backend/src frontend/src | wc -l');
    const testCount = runCommand('find backend/tests -name "*.test.ts" | wc -l');
    ...
    tasks = {
      T1_Persistence: false, T2_Auth: false, T3_Categories: false, T4_Attachments: false,
      T5_Calendar: false, T6_Email: false, T7_Performance: false, T8_ICS: false
    };
    metrics = {
      tsFileCount: 0, anyUsage: 0, consoleUsage: 0, testCount: 0, lintErrors: 0
    };
  }
...
// 4. Persist Data
fs.writeFileSync(path.join(outputDir, 'participants.json'), JSON.stringify(participants, null, 2));
fs.writeFileSync(path.join(outputDir, 'tasks_per_participant.json'), JSON.stringify(tasksPerParticipant, null, 2));
fs.writeFileSync(path.join(outputDir, 'code_quality_metrics.json'), JSON.stringify(codeQualityMetrics, null, 2));
fs.writeFileSync(path.join(outputDir, 'chat_analysis.json'), JSON.stringify(chatAnalysis, null, 2));
fs.writeFileSync(path.join(outputDir, 'survey_mapping.json'), JSON.stringify(surveyMapping, null, 2));

// Generate Summary MD
let summaryMd = "# Branch Summary\n\n";
summaryMd += "| Branch | Tasks Completed | Any Usage | Console Logs | Chat Words | Repo Exists |\n";
summaryMd += "|---|---|---|---|---|---|\n";
...
for (const p of participants) {
  ...
  summaryMd += `| ${p.id} | ${taskCount} | ${q.anyUsage} | ${q.consoleUsage} | ${c.wordCount} | ${p.hasRepo ? 'Yes' : 'No'} |\n`;
}
