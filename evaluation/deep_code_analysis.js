...
function analyzeBranch(branch) {
  ...
  // T1: Persistence
  if (checkFileContent("backend/package.json", /sqlite3/))
    branchResults.t1_persistence = "sqlite";
  else if (checkFileContent("backend/package.json", /mongoose/))
    branchResults.t1_persistence = "mongo";
  else if (checkFileContent("backend/package.json", /pg/))
    branchResults.t1_persistence = "postgres";
  else if (
    checkFileContent(
    ...
    branchResults.t1_persistence = "json-file";
  ...
  // T3: Categories
  if (
    checkFileContent("frontend/src/features/todos/TodoItem.tsx", /category/) ||
  ...
  // T4: Attachments
  if (
    checkFileContent("backend/package.json", /multer/) ||
  ...
  if (fs.existsSync("backend/package.json")) {
    try {
      ...
      try {
        ...
        branchResults.lint_errors = lintJson.reduce(
          (acc, curr) => acc + curr.errorCount,
          0
        );
      } catch (e) {
...
fs.writeFileSync(
  path.join(outputDir, "deep_code_analysis.json"),
