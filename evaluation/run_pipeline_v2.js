...
function getLocalVibeBranches() {
    try {
        const out = execSync('git branch --list "*-vibe"', { cwd: repoRoot, encoding: 'utf8' });
        return out
            .split(/\r?\n/)
...
const onlyTask2 = args.includes('--only-task2') || process.env.ONLY_TASK2 === '1';
const branchFilterArg = args.find((a) => a.startsWith('--branches='));
const branchFilter = branchFilterArg
    ? branchFilterArg.replace('--branches=', '').split(',').map((s) => s.trim()).filter(Boolean)
    : null;
...
const injectedFiles = [
    {
        ...
        content: readFileOrThrow(path.join(repoRoot, 'evaluation/tests/task2.user-accounts.test.ts')),
    },
    {
        rel: 'backend/tests/task3.categories.test.ts',
        content: readFileOrThrow(path.join(repoRoot, 'evaluation/tests/task3.categories.test.ts')),
    },
    {
        rel: 'backend/tests/task4.attachments.test.ts',
        content: readFileOrThrow(path.join(repoRoot, 'evaluation/tests/task4.attachments.test.ts')),
    },
    {
        rel: 'backend/tests/task5.calendar.test.ts',
        content: readFileOrThrow(path.join(repoRoot, 'evaluation/tests/task5.calendar.test.ts')),
    },
    {
        rel: 'backend/tests/utils/testDb.ts',
        content: readFileOrThrow(path.join(repoRoot, 'evaluation/tests/utils/testDb.ts')),
    },
    {
        rel: 'backend/tests/task1.persistent-todos.test.ts',
        content: readFileOrThrow(path.join(repoRoot, 'evaluation/tests/task1.persistent-todos.test.ts')),
    },
];
...
function analyzeBranch(branch) {
    ...
    // Install test deps if missing (fast check)
    if (fs.existsSync('backend/package.json')) {
           // Ensure dependencies for the checked-out branch are installed.
    ...
    for (const tf of testFiles) {
        ...
        if (onlyTask2 && tf.key === 'T2') {
            fs.writeFileSync(path.join(debugDir, `${branchName}.task2.out.txt`), result.output || '');
        }
    }
...
// Restore Thesis Branch and Tests
runCmd('git checkout thesis');
// We don't restore tests to thesis branch, as we want to keep the new ones there? 
// Actually, the prompt implies we refactored them ON thesis branch.
// So we should just ensure we are back.

fs.writeFileSync('evaluation/task_pipeline_v2.json', JSON.stringify(results, null, 2));

// CSV Generation
let csv = 'branch_id,attempted_T1,implemented_T1,verified_T1,attempted_T2,implemented_T2,verified_T2,attempted_T3,implemented_T3,verified_T3,attempted_T4,implemented_T4,verified_T4,attempted_T5,implemented_T5,verified_T5\n';
...
fs.writeFileSync('evaluation/task_pipeline_v2.csv', csv);
