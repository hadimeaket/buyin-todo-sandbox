...
const branches = JSON.parse(
  fs.readFileSync("evaluation/branches.json", "utf8")
);
...
function analyzeBranch(branch) {
  ...
  results.test_results[branchName] = {
    total: testCount,
    passed: passCount,
  ...
  tasks.T1 = {
    ...
    implemented:
      hasRepo &&
      (repoContent.includes("save") || repoContent.includes("create")),
    verified: hasRepo && repoContent.includes("sqlite"), // Strict check for SQLite
    strategy: repoContent.includes("sqlite")
      ? "SQLite"
      : repoContent.includes("writeFile")
      ? "JSON"
      : "Memory/Other",
  ...
  const authContent = hasAuth
    ? fs.existsSync(authFile)
      ? fs.readFileSync(authFile, "utf8")
      : fs.readFileSync(userController, "utf8")
    : "";
  ...
  tasks.T2 = {
    ...
    implemented:
      hasAuth &&
      (authContent.includes("jwt") || authContent.includes("verify")),
    verified:
      hasAuth && authContent.includes("verify") && authContent.includes("401"),
  ...
  tasks.T4 = {
    ...
    verified:
      hasAttach && checkFile("backend/src/routes/todoRoutes.ts", /multer/),
  ...
  tasks.T5 = {
    ...
    verified:
      hasCal &&
      (calContent.includes("date-fns") || calContent.includes("moment")),
...
fs.writeFileSync(
  path.join(outputDir, "lint_results.json"),
  JSON.stringify(results.lint_results, null, 2)
);
fs.writeFileSync(
  path.join(outputDir, "task_analysis.json"),
  JSON.stringify(results.task_analysis, null, 2)
);
fs.writeFileSync(
  path.join(outputDir, "examples_good_bad.json"),
  JSON.stringify(results.examples, null, 2)
);
