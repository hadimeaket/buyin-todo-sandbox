...
function getTodoId(todo: AnyObj | undefined): string | undefined {
  ...
  return (
    todo.id || todo._id || todo.todoId || todo.todo_id || todo.uuid || todo.uid
  );
}

function extractRange(
  todo: AnyObj | undefined
...
async function getFilteredTodos(
  ...
  const resIso = await request(app)
    ...
    .query({ start, end })
    .set(headers);
  if (resIso.status === 200 && Array.isArray(parseTodos(resIso.body)))
    return resIso;
  ...
  it("supports multi-day todos for calendar queries (persisted range + overlap filtering)", async () => {
    ...
    if ([200, 201].includes(eqCreate.status)) {
      ...
      if (eqTodo) {
        ...
        if (eqRange) {
          expect(eqRange.end.getTime()).not.toBeGreaterThan(
            eqRange.start.getTime()
          );
        }
      }
    }
