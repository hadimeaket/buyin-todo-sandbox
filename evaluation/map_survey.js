...
const branchesFile = "/home/cgoek/Studium/BA/buyin-todo-sandbox/evaluation/branches.txt";
...
const branches = fs.readFileSync(branchesFile, 'utf8').split('\n').filter(Boolean).concat(extraBranches);
...
rows.forEach((row, index) => {
    ...
    let matchedBranch = branches.find(b => b.toLowerCase().includes(surveyBranch.toLowerCase()) || surveyBranch.toLowerCase().includes(b.toLowerCase()));
    
    // Manual fixups if fuzzy fails
    if (!matchedBranch) {
        if (surveyBranch.includes("Max Helling") || name.includes("Helling")) matchedBranch = "helling-max-vibe";
        if (surveyBranch.includes("Sangat") || name.includes("Sethi")) matchedBranch = "sethi-sangat-vibe";
        if (name.includes("Fadime") || surveyBranch.includes("fadime")) matchedBranch = "goek-fadime-vibe";
    }
...
// Write outputs
fs.writeFileSync(path.join(__dirname, 'survey_mapping.json'), JSON.stringify(mapping, null, 2));
fs.writeFileSync(path.join(__dirname, 'survey_mapping.csv'), csvLines.join('\n'));
fs.writeFileSync(path.join(__dirname, 'branches.json'), JSON.stringify(branches, null, 2));
