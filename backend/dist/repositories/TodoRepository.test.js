...
describe('TodoRepository', () => {
    beforeEach(async () => {
        ...
        if (TodoRepository_1.todoRepository.clear) {
            await TodoRepository_1.todoRepository.clear();
        }
    });
    describe('create', () => {
    ...
    describe('update', () => {
        it('should update a todo title', async () => {
            ...
            const updated = await TodoRepository_1.todoRepository.update(created.id, TEST_USER_ID, updateDto);
        ...
        it('should return null when updating non-existent todo', async () => {
            const updated = await TodoRepository_1.todoRepository.update('non-existent', TEST_USER_ID, { title: 'Test' });
    ...
    describe('toggle', () => {
        ...
        it('should toggle todo from completed to not completed', async () => {
            const created = await TodoRepository_1.todoRepository.create({ title: 'Toggle Me' }, TEST_USER_ID);
            await TodoRepository_1.todoRepository.update(created.id, TEST_USER_ID, { completed: true });
            const toggled = await TodoRepository_1.todoRepository.toggle(created.id, TEST_USER_ID);
            expect(toggled).toBeDefined();
