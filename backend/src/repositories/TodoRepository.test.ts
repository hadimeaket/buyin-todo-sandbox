...
describe("TodoRepository", () => {
  ...
  describe("findAll", () => {
    it("should return all todos", async () => {
      const todo1 = await todoRepository.create(TEST_USER_ID, {
        title: "Todo 1",
      });
      const todo2 = await todoRepository.create(TEST_USER_ID, {
        title: "Todo 2",
      });
  ...
  describe("findById", () => {
    it("should return a todo when it exists", async () => {
      const created = await todoRepository.create(TEST_USER_ID, {
        title: "Find Me",
      });
  ...
  describe("toggle", () => {
    ...
    it("should toggle todo from completed to not completed", async () => {
      ...
      await todoRepository.update(created.id, TEST_USER_ID, {
        completed: true,
      });
