...
describe('TodoRepository', () => {
  ...
  describe('findAll', () => {
    it('should return all todos', async () => {
      const todo1 = await todoRepository.create({ title: 'Todo 1' }, testUserId);
      const todo2 = await todoRepository.create({ title: 'Todo 2' }, testUserId);
  ...
  describe('findById', () => {
    it('should return a todo when it exists', async () => {
      const created = await todoRepository.create({ title: 'Find Me' }, testUserId);
    ...
    it('should return null when todo does not exist', async () => {
      const found = await todoRepository.findById('non-existent-id', testUserId);
  ...
  describe('update', () => {
    it('should update a todo title', async () => {
      const created = await todoRepository.create({ title: 'Original' }, testUserId);
      const updateDto: UpdateTodoDto = { title: 'Updated' };
      
      const updated = await todoRepository.update(created.id, testUserId, updateDto);
    ...
    it('should update a todo completed status', async () => {
      const created = await todoRepository.create({ title: 'Test' }, testUserId);
    ...
    it('should return null when updating non-existent todo', async () => {
      const updated = await todoRepository.update('non-existent', testUserId, { title: 'Test' });
  ...
  describe('toggle', () => {
    it('should toggle todo from not completed to completed', async () => {
      const created = await todoRepository.create({ title: 'Toggle Me' }, testUserId);
  ...
  describe('delete', () => {
    it('should delete an existing todo', async () => {
      const created = await todoRepository.create({ title: 'Delete Me' }, testUserId);
